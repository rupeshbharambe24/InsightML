...
# Register the template with Plotly so `template="insightml"` works
pio.templates["insightml"] = INSIGHTML_TEMPLATE


def apply_theme() -> None:
    """Set InsightML as the default Plotly template globally."""
    pio.templates.default = "insightml"
...
def make_figure(title: str = "", **layout_kwargs) -> go.Figure:
    ...
    """
    fig = go.Figure()
    fig.update_layout(template="insightml", title=title, **layout_kwargs)
    return fig
